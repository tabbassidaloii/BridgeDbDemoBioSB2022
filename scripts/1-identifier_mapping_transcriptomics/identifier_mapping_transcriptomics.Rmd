...
# ensemblID_doubles <- names (table(ensemblID$SYMBOL)[table(ensemblID$SYMBOL) > 1])
# ensemblID %>% filter (SYMBOL %in% ensemblID_doubles) %>% arrange (SYMBOL) %>% head () # If you want to see which genes have duplicated Ensembl ID
...
rm(list = setdiff(ls(), c("dataset_UC", "dataset_CD", "entrezID_doubles", "ensemblID_doubles"))) # removing variables that are not required
```
