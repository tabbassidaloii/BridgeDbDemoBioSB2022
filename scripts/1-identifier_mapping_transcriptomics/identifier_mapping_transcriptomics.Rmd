...
please look up [here](https://bridgedb.github.io/pages/system-codes.html) for the correct system code for each datasource
...
```{r converting_PriSymbol_BridgeDb.3, warning = FALSE, message = FALSE}
...
MappingStats <- data.table(`  ` =  c("The total number of unique Entrez IDs",
                           ...
                           org.Hs = c(sum(!is.na(dataset_CD$ENTREZ.ID_org.Hs)), sum(is.na(dataset_CD$ENTREZ.ID_org.Hs)), entrezID_doubles_Hs,
                                      sum(!is.na(dataset_CD$Ensembl.ID_org.Hs)), sum(is.na(dataset_CD$Ensembl.ID_org.Hs)), ensemblID_doubles_Hs),
                           BridgeDb = c(sum(!is.na(dataset_CD$ENTREZ.ID_BridgeDb)), sum(is.na(dataset_CD$ENTREZ.ID_BridgeDb)), entrezID_doubles_BridgeDb,
                                        sum(!is.na(dataset_CD$Ensembl.ID_BridgeDb)), sum(is.na(dataset_CD$Ensembl.ID_BridgeDb)), ensemblID_doubles_BridgeDb),
                           PrimaryID_BridgeDb = c(sum(!is.na(dataset_CD$ENTREZ.ID_BridgeDb_priID)), sum(is.na(dataset_CD$ENTREZ.ID_BridgeDb_priID)), entrezID_doubles_PriID_BridgeDb,
                                                  sum(!is.na(dataset_CD$Ensembl.ID_BridgeDb_PriID)), sum(is.na(dataset_CD$Ensembl.ID_BridgeDb_PriID)), ensemblID_doubles_PriID_BridgeDb))
...
```{r writing_to_notebooks,warning = FALSE, message = FALSE, echo = FALSE}
